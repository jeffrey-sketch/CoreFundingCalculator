...
function initializeDarkMode() {
    ...
    // Listen for Theme Toggle interaction
    if(toggleCheckbox) {
...
function initializeCalculator() {
    ...
    // 根據預設州份觸發自動讀取 (Trigger the automated holiday loading based on state)
    const stateSelect = document.getElementById('stateSelect');
    if (stateSelect && appState.selectedState) {
        stateSelect.value = appState.selectedState;
    }

    // Set initial remoteness
    const remotenessSelect = document.getElementById('remotenessSelect');
    if (remotenessSelect && appState.remoteness) {
        remotenessSelect.value = appState.remoteness;
    }

    autoLoadHolidays(appState.selectedState || 'VIC');
}

// --- NEW FUNCTION: Auto-load ICS file from the server based on state ---
async function autoLoadHolidays(stateCode) {
    if (!stateCode) return;
    try {
        // 讀取對應州份的檔案，例如 holidays_vic.ics (Load corresponding state file)
        const response = await fetch(`./holidays_${stateCode.toLowerCase()}.ics`);
        
        if (response.ok) {
            const icsData = await response.text();
            const parsedDates = parseICS(icsData);
            
            if (parsedDates.length > 0) {
                // 替換現有的假期日期 (Replace instead of append so changing state clears old dates)
                appState.importedHolidays = parsedDates;
                console.log(`Successfully auto-loaded ${parsedDates.length} holidays for ${stateCode}.`);
                renderAll();
            }
        } else {
            console.log(`No holidays_${stateCode.toLowerCase()}.ics file found. Using empty holidays.`);
            appState.importedHolidays = [];
            renderAll();
        }
    } catch (error) {
        console.warn(`Auto-load skipped for ${stateCode} (normal for local file:/// usage).`);
    }
}
...
function setupEventListeners() {
    ...
    // --- Category Budget Tracking Logic ---
    const categoryBudgetInputs = document.querySelectorAll('.category-budget-input');
    categoryBudgetInputs.forEach(input => {
        input.addEventListener('input', (e) => {
            handleCurrencyInput(e);
            const categoryName = e.target.dataset.category;
            const amount = parseCurrencyString(e.target.value);
            
            // Ensure object exists
            if (!appState.categoryBudgets) appState.categoryBudgets = {};
            
            appState.categoryBudgets[categoryName] = amount;
            debouncedGenerate(); // Instantly update Summary Tables
        });
    });

    // --- Remoteness Dropdown Logic ---
    const remotenessSelectEl = document.getElementById('remotenessSelect');
    if (remotenessSelectEl) {
        remotenessSelectEl.addEventListener('change', (e) => {
            appState.remoteness = e.target.value;
            debouncedGenerate();
        });
    }

    // --- State Dropdown Logic (州份選擇邏輯) ---
    ...
    if (stateSelectEl) {
        stateSelectEl.addEventListener('change', (e) => {
            ...
            autoLoadHolidays(appState.selectedState);
        });
    }

    // --- ICS Import Logic (Local Fallback) ---
    if (importIcsBtn && icsFileInput) {
        importIcsBtn.addEventListener('click', (e) => {
            e.preventDefault();
            icsFileInput.click();
        });
        ...
        icsFileInput.addEventListener('change', (e) => {
            ...
            reader.onload = (event) => {
                ...
                const parsedDates = parseICS(icsData); // Uses function from utils.js
                
                if (parsedDates.length > 0) {
                    // 手動導入時附加日期 (Append custom dates when manually imported)
                    appState.importedHolidays = [...new Set([...(appState.importedHolidays || []), ...parsedDates])];
                    calcShowMessage(`Successfully imported ${parsedDates.length} holiday dates.`);
                    debouncedGenerate();
                } else {
                    calcShowMessage("No valid dates found in the imported .ics file.");
                }
                icsFileInput.value = ''; // Reset input
            };
...
function openScheduleModal(day, hour, serviceId = null, selection = null, context = 'weekly', instance = null) {
    ...
    const freqContainer = document.getElementById('modalFrequencyContainer');
    ...
    // Show or hide frequency based on context (it's not applicable to one-off instances)
    if (freqContainer) {
        freqContainer.style.display = (context === 'weekly') ? 'block' : 'none';
    }
    ...
    if (context === 'edit-instance' && instance) {
        ...
        if (slot) {
            ...
            if (freqEl) freqEl.value = slot.frequency || 'weekly';
...
function saveServiceFromModal() {
    ...
    if (context === 'edit-instance') {
        ...
        let slotData = {
            ...
            costDivider: parseFloat(document.getElementById('modalRatio').value) || 1,
            overrideRate: parseCurrencyString(document.getElementById('modalRate').value) || null,
            frequency: document.getElementById('modalFrequency') ? document.getElementById('modalFrequency').value : 'weekly'
        };
...
function saveCalculatorState() {
    ...
    const formData = {
        ...
        selectedState: document.getElementById('stateSelect') ? document.getElementById('stateSelect').value : 'VIC',
        remoteness: document.getElementById('remotenessSelect') ? document.getElementById('remotenessSelect').value : 'National',
    ...
    const saveData = {
        ...
        importedHolidays: appState.importedHolidays, // Save imported dates
...
function loadCalculatorState(event) {
    ...
    reader.onload = (e) => {
        try {
            ...
            if(data.formData.selectedState !== undefined) {
                const stateSel = document.getElementById('stateSelect');
                if(stateSel) stateSel.value = data.formData.selectedState;
                appState.selectedState = data.formData.selectedState;
            }
            if(data.formData.remoteness !== undefined) {
                const remSel = document.getElementById('remotenessSelect');
                if(remSel) remSel.value = data.formData.remoteness;
                appState.remoteness = data.formData.remoteness;
            }
            
            if(data.formData.categoryBudgets !== undefined) {
                appState.categoryBudgets = data.formData.categoryBudgets;
                // Pre-fill budget input fields
                document.querySelectorAll('.category-budget-input').forEach(input => {
                    const cat = input.dataset.category;
                    if(appState.categoryBudgets[cat]) {
                        input.value = formatNumber(appState.categoryBudgets[cat], false);
                    }
                });
            } else {
                appState.categoryBudgets = {}; // ensure fresh slate on old loads
            }
            ...
            appState.importedHolidays = data.importedHolidays || []; // Load custom imported dates
            
            // Backwards compatibility for old saved plans that use the combined strings
            appState.weeklyScheduleSlots.forEach(slot => {
                if(slot.serviceType === 'Self-Care Activities/Social and Community Access') {
                    slot.serviceType = 'Self-Care Activities';
                }
            });
            appState.manualServices.forEach(ms => {
                if(ms.description === 'Self-Care Activities/Social and Community Access') {
                    ms.description = 'Self-Care Activities';
                }
            });
