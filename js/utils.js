...
function parseICS(icsData) {
    ...
    for (const line of lines) {
        if (line.startsWith('BEGIN:VEVENT')) {
            ...
            // Match standard patterns like DTSTART:20241225 or DTSTART;VALUE=DATE:20241225
...
function calcIsPublicHoliday(dateObj) {
    const dateStr = calcFormatDate(dateObj);
    const isImported = appState.importedHolidays && appState.importedHolidays.includes(dateStr);
    return !!isImported;
}
...
function exportToPdf() {
    ...
    try {
        ...
        const includePH = document.getElementById('includePublicHolidays')?.checked ? 'Yes' : 'No';
        doc.text(`Funding Release Period: ${releasePeriodText}`, 14, yPos);
        doc.text(`Include Public Holidays: ${includePH}`, 105, yPos);
        yPos += 6;
        ...
        if (appState.weeklyScheduleSlots.length > 0) {
            doc.setFont("helvetica", "bold");
            doc.setFontSize(14);
            doc.text("Recurring Support Summary", 14, yPos);
            yPos += 6;
            ...
            const freqLabels = { 'weekly': 'Weekly', 'fortnightly': 'Fortnight', 'monthly': 'Month' };
            ...
            Object.values(groups).sort((a, b) => {
                ...
                summaryBody.push([
                    ...
                    `${cycleHours.toFixed(2)} Hrs / ${freqLabels[group.frequency]}`
                ]);
            });

            doc.autoTable({
                startY: yPos,
                head: [['Pattern', 'Time', 'Frequency', 'Duration', 'Cycle Total']],
                body: summaryBody,
                theme: 'grid',
                headStyles: { fillColor: [220, 220, 220], textColor: [50, 50, 50], font: "helvetica", fontStyle: "bold" },
                bodyStyles: { font: "helvetica", fontSize: 10 },
                foot: [
                    ['', '', '', 'Avg. Weekly Hours:', `${avgWeeklyGrandTotal.toFixed(2)} Hrs/Wk`]
                ],
