function renderWeeklySummary() {
    ...
    appState.weeklyScheduleSlots.forEach(slot => {
        ...
        const key = `${slot.startTime}-${slot.endTime}-${freq}`;
        if (!groups[key]) {
            groups[key] = {
    ...
    const freqLabels = { 'weekly': 'Weekly', 'fortnightly': 'Fortnightly', 'monthly': 'Monthly' };
    const freqSuffix = { 'weekly': 'Wk', 'fortnightly': 'Fn', 'monthly': 'Mo' };
    let avgWeeklyGrandTotal = 0;
    ...
    Object.values(groups).sort((a, b) => {
        ...
        const cycleHours = group.duration * count;
        
        let normalizedWeekly = cycleHours;
        if (group.frequency === 'fortnightly') normalizedWeekly = cycleHours / 2;
        if (group.frequency === 'monthly') normalizedWeekly = (cycleHours * 12) / 52;
        
        avgWeeklyGrandTotal += normalizedWeekly;
        ...
        html += `
            <tr class="hover:bg-gray-50 transition-colors">
                ...
                <td class="py-3 px-4 text-gray-600 font-medium text-xs capitalize">${freqLabels[group.frequency]}</td>
                <td class="py-3 px-4 text-center text-gray-600">${group.duration.toLocaleString('en-US', {maximumFractionDigits: 2})}h</td>
                <td class="py-3 px-4 text-right font-semibold text-gray-800">${cycleHours.toLocaleString('en-US', {maximumFractionDigits: 2})}h / ${freqSuffix[group.frequency]}</td>
            </tr>
        `;
    });

    bodyEl.innerHTML = html;
    totalEl.textContent = `${avgWeeklyGrandTotal.toLocaleString('en-US', {maximumFractionDigits: 2})} Hrs/Wk`;
}
...
function renderCalendar() {
    ...
    for(let i = 1; i <= daysInMonth; i++) {
        ...
        // Use dynamically imported custom holidays, fallback safely if undefined
        const isHoliday = appState.importedHolidays && appState.importedHolidays.includes(dayStr);
...
function renderSummaryBox() {
    ...
    const budgetContainer = document.getElementById('categoryBudgetSummaryContainer');
    ...
    // --- Category Budget vs Estimated Cost Logic ---
    const categoryTotals = {};
    // Initialize tracking object from entered budgets
    Object.keys(appState.categoryBudgets || {}).forEach(cat => {
        if(appState.categoryBudgets[cat] > 0) {
            categoryTotals[cat] = { budget: appState.categoryBudgets[cat], cost: 0 };
        }
    });
    ...
    appState.allServiceInstances.forEach(instance => {
        ...
        if (!categoryTotals[catKey]) {
            categoryTotals[catKey] = { budget: appState.categoryBudgets[catKey] || 0, cost: 0 };
        }
        categoryTotals[catKey].cost += instance.cost;

        // Populate detailed itemized summary rows below
    ...
    // 1. Draw Category Budgets Tracking Area (If data exists)
    const categoriesWithData = Object.keys(categoryTotals).filter(cat => categoryTotals[cat].budget > 0 || categoryTotals[cat].cost > 0);
    
    if (categoriesWithData.length > 0 && budgetContainer) {
        let budgetTableHtml = `
            <div class="mb-4">
                <h4 class="text-sm font-bold text-gray-800 mb-3 uppercase tracking-wider"><i class="fas fa-bullseye text-blue-500 mr-2"></i>Category Budget Tracking</h4>
                <div class="overflow-x-auto rounded-xl border border-gray-200">
                    <table class="w-full text-sm">
                        <thead class="bg-blue-50/50 text-gray-600 font-medium border-b border-gray-200">
                            <tr>
                                <th class="py-2.5 px-4 text-left">Category</th>
                                <th class="py-2.5 px-4 text-right border-l border-gray-100">Allocated Budget</th>
                                <th class="py-2.5 px-4 text-right border-l border-gray-100">Estimated Cost</th>
                                <th class="py-2.5 px-4 text-right border-l border-gray-100 bg-gray-50 font-bold">Remaining</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-100">
        `;

        categoriesWithData.sort().forEach(cat => {
            const data = categoryTotals[cat];
            const remaining = data.budget > 0 ? data.budget - data.cost : null;
            const remainingClass = remaining !== null ? (remaining >= 0 ? 'text-green-600' : 'text-red-600') : 'text-gray-400 font-normal';
            const remainingText = remaining !== null ? formatNumber(remaining, true) : 'N/A (No Budget Set)';
            
            // Re-map nice name if necessary
            let niceName = serviceTypeMap[cat] ? serviceTypeMap[cat] : cat;

            budgetTableHtml += `
                <tr class="hover:bg-gray-50 transition-colors">
                    <td class="py-2.5 px-4 font-semibold text-gray-700">${niceName}</td>
                    <td class="py-2.5 px-4 text-right text-gray-600 border-l border-gray-50">${data.budget > 0 ? formatNumber(data.budget, true) : '-'}</td>
                    <td class="py-2.5 px-4 text-right font-semibold text-gray-800 border-l border-gray-50">${formatNumber(data.cost, true)}</td>
                    <td class="py-2.5 px-4 text-right font-bold ${remainingClass} border-l border-gray-50 bg-gray-50/50">${remainingText}</td>
                </tr>
            `;
        });

        budgetTableHtml += `</tbody></table></div></div>`;
        budgetContainer.innerHTML = budgetTableHtml;
    }


    // 2. Draw Detail Itemized Rows
    let tableRowsHtml = '';
