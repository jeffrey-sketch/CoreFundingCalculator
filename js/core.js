...
// Helper function to apply Remote/Very Remote loadings based on the user's selected state
function applyRemoteness(rateObj) {
    if (!rateObj) return rateObj;
    
    let finalRate = rateObj.rate;
    // Safely get the current remoteness, default to National if undefined
    const currentRemoteness = (typeof appState !== 'undefined' && appState.remoteness) ? appState.remoteness : 'National';

    if (currentRemoteness === 'Remote' && rateObj.rateRemote) {
        finalRate = rateObj.rateRemote;
    } else if (currentRemoteness === 'Very Remote' && rateObj.rateVeryRemote) {
        finalRate = rateObj.rateVeryRemote;
    }
    
    // Return a cloned object so we don't mutate the original config dictionary
    return { ...rateObj, rate: finalRate };
}
...
function getRateInfo(serviceType, dateTimePoint) {
    ...
    if (serviceRateInfo["Any"]) return { rateObj: applyRemoteness(serviceRateInfo["Any"]), rateTypeString: 'Flat Rate' };

    const dayIndex = serviceDate.getDay();
    if (dayIndex === 6) return { rateObj: applyRemoteness(serviceRateInfo["Sat"]), rateTypeString: 'Sat' };
    if (dayIndex === 0) return { rateObj: applyRemoteness(serviceRateInfo["Sun"]), rateTypeString: 'Sun' };

    if (currentHour >= 0 && currentHour < 6) return { rateObj: applyRemoteness(serviceRateInfo["Mon-Fri-Night"]), rateTypeString: 'Mon-Fri-Night' };
    if (currentHour >= 6 && currentHour < 20) return { rateObj: applyRemoteness(serviceRateInfo["Mon-Fri-Day"]), rateTypeString: 'Mon-Fri-Day' };
    if (currentHour >= 20 && currentHour < 24) return { rateObj: applyRemoteness(serviceRateInfo["Mon-Fri-Evening"]), rateTypeString: 'Mon-Fri-Evening' };
...
function calcGetRateForService(serviceType, dateTimePoint) {
    ...
    if (calcIsPublicHoliday(serviceDate) && serviceRateInfoSet && serviceRateInfoSet["Public Holiday"]) {
        return { ...applyRemoteness(serviceRateInfoSet["Public Holiday"]), rateType: 'Public Holiday' };
    }
...
function calcGetStandardRateForService(serviceType, dateTimePoint) {
     ...
     // Check if it's a public holiday first to give accurate placeholder rates
     if (calcIsPublicHoliday(serviceDate) && serviceRateInfoSet && serviceRateInfoSet["Public Holiday"]) {
         return applyRemoteness(serviceRateInfoSet["Public Holiday"]);
     }
...
function calculateAndCreateInstance(brokenService, date, type, baseId) {
    ...
    return {
        ...
        baseServiceType: brokenService.serviceType || 'Manual', // Used for category budgeting tracking
        details: detailsText,
...
function breakdownService(service) {
    ...
    const typesNeedingBreakdown = ['Self-Care Activities', 'Social and Community Access', 'Self-Care Activities/Social and Community Access'];
    if (!isWeekday || !typesNeedingBreakdown.includes(service.serviceType)) {
            ...
            if (manualService.startTime && manualService.endTime) {
                ...
                brokenDownManuals.forEach(brokenService => {
                 ...
                 if (currentRates[type] && currentRates[type].Any) {
                    // Apply remoteness formatting to visual strings on manual items as well
                    const adjustedRateObj = applyRemoteness(currentRates[type].Any);
                    rateDisplay = '$' + adjustedRateObj.rate.toFixed(2) + '/' + adjustedRateObj.unit;
                 }
                 ...
                 appState.allServiceInstances.push({
                    ...
                    baseServiceType: type, // Tracked for individual budgets
